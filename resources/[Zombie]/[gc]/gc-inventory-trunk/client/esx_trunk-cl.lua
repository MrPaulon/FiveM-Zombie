        ...
        if vehFront > 0 and closecar ~= nil and GetPedInVehicleSeat(closecar, -1) ~= GetPlayerPed(-1) then
          ...
          if ESX.UI.Menu.IsOpen("default", GetCurrentResourceName(), "inventory") then
            ...
            if locked == 1 or class == 15 or class == 16 or class == 14 then
              ...
              exports['dilza-notify']:Alert("Information", _U('trunk_closed'), 5000, 'error')    
            end
          end
        else
          exports['dilza-notify']:Alert("Information", _U('no_veh_nearby'), 5000, 'error')    
        end
        lastOpen = true
        GUI.Time = GetGameTimer()
      end
    else
        exports['dilza-notify']:Alert("Information", _U('nacho_veh'), 5000, 'error')    
    end
  end
end
local count = 0
