...
AddEventHandler('gc-removeweapon', function(itemname,count)
	...
	TriggerClientEvent('dilza-notify:Alert', _source, "Information", "Votre objet est cassé", 5000, 'warning')	
end)
...
AddEventHandler(
	...
	--			TriggerEvent("tqrp_base:serverlog", "[TRADEWEAPON]: " .. GetPlayerName(_source) .. " ("..itemName .. ") x" .. itemCount .. " to " .. GetPlayerName(target), _source, GetCurrentResourceName())
				...
				TriggerClientEvent('dilza-notify:Alert', _source, "Information", "Le joueur à déjà cette arme", 5000, 'error')	
			end
		end
	end
)
...
AddEventHandler("suku:SellItemToPlayer",function(source, type, item, count)
				...
				if list[i].name == item then
					...
					if item =='WEAPON_ASSAULTRIFLE' then
						...
						if xPlayer.job.name == 'police' then
							...
							if xPlayer.getInventoryItem(recipe1).count >= 1 and xPlayer.getInventoryItem(recipe2).count >= 10 then
								...
								TriggerClientEvent('dilza-notify:Alert', source, "Information", "Vous devez avoir les items suivants pour fabriquer cet item 1.Blueprint du pistolet Ak 2.Metal", 5000, 'error')	
							end
						end
					elseif item == 'WEAPON_PUMPSHOTGUN' then
						...
						if xPlayer.job.name == 'police' then
							...
							TriggerClientEvent('dilza-notify:Alert', source, "Information", "Vous devez avoir les items suivants pour fabriquer cet item 1.Pumpshot Gun Blueprint 2.Metal", 5000, 'error')
							end
						end
					else
					 if xPlayer.getMoney() >= totalPrice then
						...
						TriggerClientEvent('dilza-notify:Alert', source, "Information", 'Vous avez acheté '..count.." "..list[i].label, 5000, 'success')
					 else
						TriggerClientEvent('dilza-notify:Alert', source, "Information", "Vous n'avez pas assez d'argent", 5000, 'error')
					 end
				end
					end
			end
	...
	if type == "item_weapon" then
		...
		if xPlayer.canCarryItem(item, count) then
			...
			TriggerClientEvent('dilza-notify:Alert', source, "Information", "Vous ne pouvez pas charger autre chose dans l'inventaire !", 5000, 'error')
		end
	end
		...
		for i = 1, #list, 1 do
			if list[i].name == item then
				...
				--		TriggerEvent("tqrp_base:serverlog", "[BUYAMMO]: ("..list[i].weaponhash .. ") x" .. ammo .. " $("..totalPrice..")", _source, GetCurrentResourceName())
					...
					TriggerClientEvent('dilza-notify:Alert', source, "Information", "Vous n'avez pas l'arme pour ce type de munitions. !", 5000, 'error')
				end
			end
		end
	end
