...
RegisterNUICallback("PutIntoGlovebox", function(data, cb)
        if type(data.number) == "number" and math.floor(data.number) == data.number then
            ...
            TriggerServerEvent("esx_glovebox:putItem", gloveboxData.plate, data.item.type, data.item.name, count, gloveboxData.max, 1, data.item.label)
        end
...
RegisterNUICallback("TakeFromGlovebox", function(data, cb)
        if type(data.number) == "number" and math.floor(data.number) == data.number then
            TriggerServerEvent("esx_glovebox:getItem", gloveboxData.plate, data.item.type, data.item.name, tonumber(data.number), gloveboxData.max, 1)
        end
