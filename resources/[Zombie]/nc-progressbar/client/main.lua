...
function Process(action, start, tick, finish)
    ...
    if not IsEntityDead(PlayerPedId()) or Action.useWhileDead then
        if not isDoingAction then
            ...
            exports['dilza-notify']:Alert("INFO", "Vous êtes déjà en train de faire quelque chose!", 5000,'info')
        end
    else
        exports['dilza-notify']:Alert("INFO", "Vous ne pouvez pas faire cette action !", 5000,'info')
    end
end
