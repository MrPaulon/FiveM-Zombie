...
if ESX.GetConfig().Multichar then
	...
	function CharacterOptions(Characters, slots, SelectedCharacter)
		local elements = {{title = "Personnage: ".. Characters[SelectedCharacter.value].firstname .. " ".. Characters[SelectedCharacter.value].lastname,icon = "fa-regular fa-user", unselectable = true}, 
		{title = "Retour", unselectable = false,icon = "fa-solid fa-arrow-left",description ="Retour à la sélection des personnages.", action = "return"}}
		if not Characters[SelectedCharacter.value].disabled then 
			elements[3] = {title = _('char_play'), description ="Rejoindre le serveur.", icon ="fa-solid fa-play",action = 'play', value = SelectedCharacter.value}
		else
			elements[3] = {title = _('char_disabled'), value = SelectedCharacter.value, icon ="fa-solid fa-xmark", description ="Ce personnage est inutilisable.",}
		end
		if Config.CanDelete then elements[4] = {title = _('char_delete'),icon ="fa-solid fa-xmark",description ="Supprimer définitivement ce personnage.", action = 'delete', value = SelectedCharacter.value} end
		ESX.OpenContext("left", elements, function(element, Action)
			if Action.action == "play" then
				...
				ESX.CloseContext()
				ExecuteCommand('hud')
				TriggerEvent('mrp-hud:setMiniMap')
	...
	function SelectCharacterMenu(Characters, slots)
		...
		local elements = {{title = "Choisier un personnage." , icon = "fa-solid fa-users", description = "Sélectionner un personnage avec lequel vous voulez jouer." , unselectable = true}}
